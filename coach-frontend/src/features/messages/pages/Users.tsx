...
const UserItem: React.FC<IUserItemProps> = ({
  ...
  const containerStyle = `flex items-center p-4 gap-4 h-18.5
  ...
  return (
    <div className={containerStyle}>
      <div className='flex w-10 h-10 rounded-full'>
        {user.avatarUrl ? (
          <Image
            src={'http://5.9.85.28:8080' + user.avatarUrl}
            width={40}
            height={40}
            alt=''
      ...
      <div className='flex justify-start gap-4'>
        ...
        {user.unreadCount && (
          <div className='flex justify-center items-center w-5 h-5 bg-yellow'>
            <p className='text-black text-xxs font-medium'>{user.unreadCount}</p>
          </div>
        )}
      </div>
    </div>
